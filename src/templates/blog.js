...
const BlogPage = ({ pageContext }) => {
  ...
  return (
    <Layout>
      ...
      <Header title="Collecting Our Thoughts">
        <h3>
          We think it's important to share and reflect what we've learned over
          the years. Here's a collection of posts around who we are and what
          we've done.
        </h3>
      </Header>
          ...
          .map(({ node }) => {
            return (
              <ContentBlock vertical={true} key={node.id}>
                <Link
                  to={node.fields.slug}
                  className={BlogPostStyles.preview__link}
                >
                  <div className={BlogPostStyles.preview__imageWrapper}>
                    <img
                      ...
                      alt="generic stock of something semi related to blog post"
                    />
                  </div>
